...
def compute_average_strides_participant(participant_data):

    # extract the 'strides' data structure from the participant data structure
    ...
    for file_name in stride_data:
        ...
        # create a new data structure to hold average signals for this trial
        file_averages = {}

        # we'll store average data in a pandas DataFrame with these columns:
        ...
        for signal_name in trial:
            ...
            # put the DataFrame in a dictionary where the file name is the key
            file_averages[signal_name] = df

        # once all signals have been iterated over, put the averages for all
        # signals in a dictionary where the file name is the key
...
def average_strides(data):
    ...
    for participant in data:
        compute_average_strides_participant(data[participant])
